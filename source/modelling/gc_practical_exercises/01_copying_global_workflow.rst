...
for you to copy. For a regional workflow example please use
``|regional_workflow_example|``, for a global workflow example please use
``|global_workflow_example|``.

You can copy or check out an existing workflow using the ``rosie`` command-line
tool, replacing ``|global_workflow_example|`` with the actual suite ID.
...
.. tab-set::
   ...
   .. tab-item:: Met Office
      ...
      .. code-block:: bash
         ...
         rosie copy |global_workflow_example|
...
.. tab-set::
   ...
   .. tab-item:: Met Office
      ...
      .. code-block:: console
         ...
         $ rosie copy |global_workflow_example|
         Copy "|global_workflow_example|/trunk@123456" to "u-?????"?
         [y or n (default)]
...
Congratulations, you have successfully copied workflow
``|global_workflow_example|`` and made the new suite ID ``u-?????``. Make a
note of that new ID: the rest of this practical refers to it as ``<suite-id>``.
You can now navigate to the workflow directory and start working on it.
