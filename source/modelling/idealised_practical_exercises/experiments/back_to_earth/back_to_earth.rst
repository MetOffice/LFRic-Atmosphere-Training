...
In this experiment, we go back to the Earth atmospheric composition
and test if convective mixing in this suite behaves in a familiar way.

Step 1: Change the vertical extent of the model
-----------------------------------------------

In the top-level configuration, change the ``LFRIC_LEVS``
to ``uniform_l100_75km``.
...
.. figure:: /_static/idealised_lfric_levs.png
   :width: 500px
   :align: center
   :alt: Rose GUI showing the top-level panel in the extraterrestrial suite.
...
experiments and compare them to each other.
